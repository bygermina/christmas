...
export const FireEffect = ({ variant = 'white', performanceMode = false }: FireEffectProps) => {
  const { screenWidth } = useScreenSizeContext();
  const isGold = variant === 'gold';

  const scale = useMemo(
    () =>
      Math.max(SCALE_CONFIG.MIN, Math.min(SCALE_CONFIG.MAX, screenWidth / SCALE_CONFIG.BASE_WIDTH)),
    [screenWidth],
  );
